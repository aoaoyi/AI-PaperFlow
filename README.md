...
会议源会先从 DBLP 获取题录、作者、DBLP 链接以及可用的 DOI/出版社链接。对筛选后的会议论文，系统会再按标题依次查询 arXiv、OpenAlex、Semantic Scholar 和 Crossref；如果标题能可靠匹配且来源提供摘要，就回填摘要、PDF 链接和分类，再进入中文分析流程。没有找到摘要的会议论文不会默认让模型凭标题猜创新点。
...
| `ARXIV_QUERY_MODE` | `broad` | `broad` 用关键词或分类的单次宽查询，减少漏抓；`strict` 使用关键词和分类同时匹配 |
...
| `CONFERENCE_ABSTRACT_SOURCES` | `arxiv,openalex,semantic_scholar,crossref` | 顶会论文摘要回填来源顺序；OpenAlex 默认排在 Semantic Scholar 前面以减少 429 |
